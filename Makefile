...
build-acceptance-servers: ## Build Acceptance Servers
	...
	@docker build backend -t interlegis/gitec.interlegis.leg.br-backend:acceptance -f backend/Dockerfile.acceptance
	@echo "Build acceptance frontend"
	@docker build frontend -t interlegis/gitec.interlegis.leg.br-frontend:acceptance -f frontend/Dockerfile

.PHONY: start-acceptance-servers
start-acceptance-servers: build-acceptance-servers ## Start Acceptance Servers
	@echo "Start acceptance backend"
	@docker run --rm -p 55001:55001 --name gitec.interlegis.leg.br-backend-acceptance -d interlegis/gitec.interlegis.leg.br-backend:acceptance
	@echo "Start acceptance frontend"
	@docker run --rm -p 3000:3000 --name gitec.interlegis.leg.br-frontend-acceptance --link gitec.interlegis.leg.br-backend-acceptance:backend -e RAZZLE_API_PATH=http://localhost:55001/plone -e RAZZLE_INTERNAL_API_PATH=http://backend:55001/plone -d interlegis/gitec.interlegis.leg.br-frontend:acceptance

.PHONY: stop-acceptance-servers
stop-acceptance-servers: ## Stop Acceptance Servers
	@echo "Stop acceptance containers"
	@docker stop gitec.interlegis.leg.br-frontend-acceptance
	@docker stop gitec.interlegis.leg.br-backend-acceptance
